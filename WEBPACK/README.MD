...
1.  **Webpack 是什么？它解决了什么问题？**
    ...
    - **是什么：** Webpack 是一个现代 JavaScript 应用程序的**静态模块打包器 (Static Module Bundler)**。它会从一个或多个入口点开始，构建一个**依赖图 (dependency graph)**，将项目中的所有模块（JS、CSS、图片、字体等）**及其依赖**打包成一个或多个最终的 bundle 文件，供浏览器加载。
    - **解决了什么问题：**
